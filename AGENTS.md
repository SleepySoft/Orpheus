...
- **Access Bridge（设备调音）**：工程顶层 `bridges` 是部署事实，画布无端口「访问桥」节点是配置投影；legacy `uart_link` 自动迁移。消息层=§18 二进制信封，当前 `uart + olink` Adapter 已贯通 `SerialSession`/UI；逻辑 `resource` 由 Target Profile 绑定实际驱动。设计见 `docs/design_access_bridge.md` / `docs/design_serial_link.md`。
...
cd ui; npm run test:e2e               # Playwright 核心流程（自动启动 :8000）
python scripts/benchmark_graph.py --nodes 128 --blocks 1000  # 大图编译/块延迟基线
