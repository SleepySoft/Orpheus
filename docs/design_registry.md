...
## 20. 信号类别与机制缺口（2026-08-09 讨论记录）

> 背景：音频系统不止"滤波器"一档。除 LTI 滤波外还有频域处理、自适应/统计估计、
> 空间音频、非线性动态、感知模型、分析/侧链控制。讨论结论：**一半能靠现有组件机制
> 表达，一半（估计/控制那半）缺图级机制**。
...
| 类别 | 典型块 | 现有机制能否表达 |
|---|---|---|
| 滤波（IIR/FIR/EQ，LTI） | biquad/fir/iir_bank | 能（音频 in/out + 系数 BULK） |
| 频域处理（FFT/STFT/相位） | rfft/ifft/FDP | 能（半复数打包的 f32 端口），但**表示是隐式约定**，无类型化 |
| 非线性/动态（状态ful 非 LTI） | limiter/soft_clipper/saturation/gain_ramper | 能（已有） |
| 感知模型（响度/掩蔽） | sleeping_beauty | 能（LUT + 状态机 + 增益斜坡，已有） |
| 空间音频（HRTF/波束/串扰消除） | holigram/input_mixer_3d | 能（几何延迟 + 矩阵混合） |
| 自适应/统计估计（相干/PSD/Wiener/LMS） | coherence_matrix、Audiopilot | **单组件内闭环能**；图级侧链不能 |
| 分析/侧链控制（电平/相干/车速/脉冲检测） | level_detect、speed_bounds | 产出的是控制信号；**图内投递不能** |

### 20.2 缺口定位

- ABI 端口类型枚举**已预留** `ORPHEUS_PORT_CONTROL` / `ORPHEUS_PORT_BULK` /
  `ORPHEUS_PORT_DEBUG`，但全仓库无组件使用，编译器/运行时也不处理——空枚举；
- 组件间只有 audio f32 数据流；控制值只能经 PROBE/readback 或外部消息（RTC/TUNE）暴露，
  **没有"组件 A 的估计输出 → 组件 B 的参数"的图内投递**；
- 图不允许反馈环（音频依赖无环）；"估计 → 调参"本质是**受控参数回灌**，不是音频环。

### 20.3 机制演进（按侵入度排序）

1. **控制端口落地**：启用 `ORPHEUS_PORT_CONTROL`——f32 标量/小向量缓冲，编译器校验
   control→control 连接并分配缓冲，运行时按块传递；语义=每块产出控制值。
2. **参数投递/侧链绑定**：控制输出可绑定目标组件参数槽（sidechain → param），
   运行时在块边界写入目标槽（复用 BULK 双 bank 原子提交思路，块边界生效）；
   保持"图无音频环"约束，控制回灌是数据依赖不是音频依赖。
3. **端口 dtype/格式标签**：端口带格式（audio f32 / half-complex / control scalar/vector），
   编译期校验兼容——让频域组件组合安全，不再靠隐式约定。
4. **任务桥（远期）**：跨速率域（TID）的估计→控制回灌语义（见 §14.3/§14.4）。

### 20.4 对蒸馏的意义

"蒸馏到基本组件组合会卡住"的根因：滤波那半可拼，估计/控制那半是另一套数学
（统计 + 线性代数），需要的是**图级控制/参数投递语义**而非更多滤波器组件。
落地 20.3 的 1+2 后，coherence_matrix → 自适应滤波这类侧链就能以图表达：
分析组件出控制信号，绑定到主链组件的参数槽。

## 21. 平台集成/外部接口节点（2026-08-09 讨论记录）

> 动机：控制面（§20）是大机制。在完整落地前，先用**声明式平台节点**覆盖非音频 DSP 的
> 集成需求（amixer、通信、传感器等）：节点可见于画布、参与代码生成，但（无音频输入的）
> 不参与 runtime 执行。这是 embed_in/embed_out「生成代码 + 用户填充」模式的泛化，
> 不是新概念。

### 21.1 节点语义

- 类别：`platform`（平台资源/外部接口）。可有音频输入（观测汇）或没有；**无音频输出**。
- **无音频输入**：纯声明，runtime 不加载不调用，仅代码生成
  （结构体 + `init/read/write` + USER CODE 段），供用户程序引用。
- **有音频输入**：v1 按 no-op 汇执行（runtime 调用、消费输入缓冲、可带 PROBE 观测），
  保证上游 buffer 语义成立；后期控制面落地后可转为"观测 → 控制输出"。
- 编译器规则：`runtime: none` 的节点不进入执行计划（纯声明层），但进入生成产物；
  有音频输入的进入计划、不产生音频输出。

### 21.2 manifest 与兼容

- 标记：`kind: platform`（或 `execution: none`）+ codegen 模板引用 + `platforms` 标签
  （PC 上 inert/解析为空，接 §19 平台解析）。
- **预留控制面扩展位**：未来该节点可声明控制输出/参数绑定（§20.3），格式不做成死路。

### 21.3 落地建议

- 复用 embed_in/out 的 platform_io.c 生成机制，泛化为 `platform_*` 模板；
- 先做一个通用 `platform_hook`（名称/接口参数化）+ 1-2 个具体示例（amixer/通信）验证链路；
- UI 单独标识平台资源节点（不参与运行）。
