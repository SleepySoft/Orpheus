...
- 音频链路使用青绿色实线与圆形 Handle；输入为绿色、输出为蓝色。控制链路使用琥珀色动态虚线、方形 Handle 和独立控制坞，并在开启“控制链路”后显示形状标签与图例。
- 子组件定义画布内，只有被 `ports[].maps_to` 导出的原子端点显示为电路图式“长引线 + 外端空心圆”（棒棒糖引脚）；被 `public_parameters[].maps_to` 导出的参数显示为“长引线 + 外端空心方块”。主图子组件实例及未导出的原子端点保持普通样式。
- 点击任一内部导出引脚会打开右侧接口面板，滚动到对应外部接口条目并闪烁提示；重复点击同一引脚会重新触发提示。
- 控制 Handle 按开关动态挂载后会显式通知 React Flow 重新测量节点，保证已存在的控制边立即获得端点并显示。
