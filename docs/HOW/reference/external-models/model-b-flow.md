# 参考工程 B 完整信号流（基于 Model_1_1.c）

> 来源：`外部参考工程 B/components/外部参考模型/src/out/baremetalgxp/slx/code/Model_1_1_ert_shrlib_rtw/Model_1_1.c`（20 641 行）及同目录 PostProcess 生成文件
...
Model_1_1_step0()  @ 0.6667 ms  (TID0, 主实时链)
  ├─ 示例工程Part2FdpFullRateTID0()   // FDP 输入缓冲 + 输出缓冲
  ├─ 示例工程Part3FullRateMixing()    // 13ch -> 22ch 矩阵混音
  ├─ 示例工程Part4FullRatePeripheralEq() // 22ch IIR + 延迟
  ├─ 示例工程Part5FullRatePostHoligram() // SleepingBeauty + 相位对齐延迟
  ├─ 示例工程Part6SummationAndPreAmp()   // 求和 + 扬声器延迟 + 淡入淡出
...
Model_1_1_step2()  @ 2.6667 ms  (TID2)
  └─ 示例工程Part2FdpFullRateTID2()   // FDP FFT 慢链：2ch -> 6ch
...
flowchart LR
    subgraph inputs [外部输入]
        ...
        TrebleLr[示例工程Part1Bands_TrebleLr 2ch]
        Mono[示例工程Part1Bands_Mono 1ch]
    end

    subgraph example-b [示例工程 全速率链]
        FDP[FDP<br/>2ch -> 6ch]
...
### 3.5 示例工程 Part3 FullRate Mixing

```text
TrebleSurround[7ch] -> 示例工程AlignmentDelay[7ch]
...
BufferOut[6ch] + audioOut_o[7ch] = MatrixConcatenate1[13ch]
  -> 示例工程FullRateMixEq IIR (software pooliirSplitProcess, 13ch)
  -> InputOrganizer:
...
Part5:
  Merge[22ch] -> sleepingBeautyProcess (4 rampers: L/R/C/Mono)
              -> 示例工程TunableDelay (PhaseAlignmentDelays[22])
              -> Buffer[22ch]

Part6:
  Buffer[22ch] + 示例工程Part5DeciRatePostHoligram_AudioOut[22ch]
              -> Add[22ch]
              -> SpeakerDelay[22ch]
              -> 示例工程OutputRouter (routing map 22->22)
...
当前 Orpheus 只有 `orpheus.builtin.iir_bank`，它假设所有通道共享同一组级联系数。示例工程 中大量实例需要**每通道不同系数**或**每通道不同级数**。因此要么扩展 `iir_bank`，要么每个通道使用独立 `iir_bank`。
...
| 4 | Model_1_1.c:12473 | `pooliirSplitProcess` | 13 | 示例工程 Part3 MixEq，软件 pooliir，每通道不同 | 13x `iir_bank` 或扩展 |
| 5 | Model_1_1.c:13297 | `iir_accelerator_process` | 22 | 示例工程 Part4 PeripheralEQ | 22x `iir_bank` 或扩展 |
...
## 5. 与当前 `example-b-step0.yaml` 中 `model_tree` 的对照
...
| **PostProcess 位置** | 在 PreAmpPart1 之后，处理 32ch Buffer | 在 示例工程 链末尾，处理 22ch |
...
- `示例工程TrebleDelay`
- `示例工程AlignmentDelay` / `TrebleSurroundDelay`
- `ChannelDelay` (Part4)
- `示例工程TunableDelay` / `FullRateHoligramDelay`
- `SpeakerDelay`
...
4. **完善 `example-b-step0.yaml` 的 `model_tree`**：
   - 按真实调用顺序排列子系统。
