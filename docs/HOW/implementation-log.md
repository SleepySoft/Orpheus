...
- 十字光标默认只在 panel/expanded 模式启用，避免小节点视觉噪音。
- 增加坐标反解纯函数测试；现有 Jest、生产构建和 Playwright 核心流程均通过。

## 2026-09-08（第五十七次：共用绘图组件覆盖特殊图表）

- coherence_matrix 的节点监控迁移到共用 Plot，新增 heatmap 序列渲染和通道轴。
- Plot 实现大面板图例；低相干到高相干保持青色到红色的颜色映射。
- 移除 nodeWidgets 中残留的旧 Canvas 尺寸 Hook，所有绘图节点已统一到共享绘图组件。
- 现有 Jest、生产构建和 Playwright 核心流程均通过。
...
- 平台专属节点增加文字徽标与左侧色条：Windows 为蓝色、DSP 为琥珀色、多平台/其它平台为青绿色；可移植节点保持中性。
- 不引入 Variant Region、差异传播或新工程字段，后端 alter 的同接口、单槽位和平台解析语义保持不变。
- 新增纯函数测试及 Playwright 几何/拖动/截图验证。
...
- 明确“离线运行”是旧称，现称“无设备批处理/主动推进运行”；“真实时长”不是第三种执行实现或硬实时，也不属于 source 组件参数。
- 明确设备/主机时钟来源由有效图自动推导；串口只建立 Access Bridge 会话，不在 PC 启动另一份图。
- 新增 `design_execution_model.md` 作为 UI、API、架构与组件文档的统一术语依据，并同步 README、WHAT、HOW、AGENTS 与仓库技能参考。
- 新增 `design_timeline.md` 审计时间线现状：当前 timestamp 恒 0、Task 近似全局单速率、无 epoch/EOS/延迟汇总/漂移校正；给出绝对 frame index、跨域时间戳和采样级事件的 P0-P3 路线。
