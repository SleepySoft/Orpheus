...
| `duration_s` | float | 10.0 s | 离线运行时长（只影响「▶ 运行」跑多久，组件本身连续发声）。0=宿主默认 10s；图中有 wav/mp3 输入时以文件全长为准。想一直听，请用「实时运行」（挂 device_out）。 |
...
- 组件本身是连续发声的；「跑一下就退出」是离线宿主的停止判据决定的（见 `duration_s`）。
