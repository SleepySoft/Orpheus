...
#define LN_FLOOR_DB (-120.0f)
#define LN_EPSILON (1.0e-12f)
...
static const OrpheusParameter params[] = {
    ...
    { .id="channels", .name="通道数", .type=ORPHEUS_VALUE_INT,
      ...
      .min_i32=1, .max_i32=LOUDNESS_NORMALIZER_MAX_CHANNELS,
      .update_policy=ORPHEUS_UPDATE_RESTART_REQUIRED, .readback=true,
      .persistent=true, .affects_signature=true },
    { .id="input_db", .name="当前输入 RMS", .type=ORPHEUS_VALUE_FLOAT,
      .default_value={.type=ORPHEUS_VALUE_FLOAT, .value.f32=LN_FLOOR_DB},
      .unit="dBFS", .update_policy=ORPHEUS_UPDATE_IMMEDIATE, .readback=true },
    { .id="gain_db", .name="当前自动增益", .type=ORPHEUS_VALUE_FLOAT,
      .default_value={.type=ORPHEUS_VALUE_FLOAT, .value.f32=0.0f},
      .unit="dB", .update_policy=ORPHEUS_UPDATE_IMMEDIATE, .readback=true },
    { .id="output_db", .name="估算输出 RMS", .type=ORPHEUS_VALUE_FLOAT,
      .default_value={.type=ORPHEUS_VALUE_FLOAT, .value.f32=LN_FLOOR_DB},
      .unit="dBFS", .update_policy=ORPHEUS_UPDATE_IMMEDIATE, .readback=true }
};
...
static const OrpheusComponentDescriptor descriptor = {
    .id="orpheus.builtin.loudness_normalizer", .version="1.0.0",
    .abi_version=ORPHEUS_ABI_VERSION, .ports=ports, .port_count=2,
    .params=params, .param_count=12, .state_size=sizeof(LoudnessNormalizerState),
...
static int create(void** state, const OrpheusConfig* config) {
    ...
    *state = calloc(1, sizeof(LoudnessNormalizerState));
...
static int prepare(void* state, const OrpheusConfig* config) {
    LoudnessNormalizerState* s = (LoudnessNormalizerState*)state;
    ...
    if (s->channels > LOUDNESS_NORMALIZER_MAX_CHANNELS) {
        s->channels = LOUDNESS_NORMALIZER_MAX_CHANNELS;
    }
    s->detector_energy = 0.0f;
    s->gain_db = 0.0f;
    s->input_db = LN_FLOOR_DB;
    s->output_db = LN_FLOOR_DB;
    return ORPHEUS_OK;
}
...
static int process(void* state, const OrpheusProcessContext* ctx) {
    ...
    const uint32_t frames = ctx->frame_count;
    ...
    for (size_t i = 0; i < sample_count; ++i) {
        block_energy += source[i] * source[i];
    }
    block_energy /= (float)sample_count;
    const float block_db = energy_to_db(block_energy);

    if (s->detector_energy <= LN_EPSILON && block_energy > LN_EPSILON) {
        s->detector_energy = block_energy;
    } else {
        const float detector_ms = block_energy > s->detector_energy
            ? s->detector_attack_ms : s->detector_release_ms;
        const float detector_coeff = block_coeff(
            detector_ms, frames, ctx->sample_rate);
        s->detector_energy += detector_coeff * (block_energy - s->detector_energy);
    }
    s->input_db = energy_to_db(s->detector_energy);

    float desired_gain_db = s->gain_db;
    if (block_db >= s->gate_db) {
    ...
    const float old_gain_db = s->gain_db;
    const float gain_ms = desired_gain_db < old_gain_db
        ? s->gain_attack_ms : s->gain_release_ms;
    const float gain_coeff = block_coeff(gain_ms, frames, ctx->sample_rate);
    s->gain_db += gain_coeff * (desired_gain_db - s->gain_db);
    s->output_db = s->input_db > LN_FLOOR_DB
        ? s->input_db + s->gain_db : LN_FLOOR_DB;

    const float old_linear = db_to_linear(old_gain_db);
    const float new_linear = db_to_linear(s->gain_db);
    const float step = (new_linear - old_linear) / (float)frames;
    float gain = old_linear;
...
static int set_parameter(void* state, const char* id, const OrpheusValue* value) {
    ...
    if (strcmp(id, "target_db") == 0) s->target_db = value->value.f32;
...
static int get_parameter(void* state, const char* id, OrpheusValue* value) {
    ...
    if (strcmp(id, "target_db") == 0) value->value.f32 = s->target_db;
