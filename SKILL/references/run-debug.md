...
- `POST /api/projects/{name}/run`：动态 Runtime 执行。含 device_in/device_out → 外部声卡 callback 触发（rt_host 子进程）；否则执行器主动推进，默认全速跑完即退出。兼容响应字段仍为 `mode: realtime|offline`。
...
- UI：「▶ 运行」= 动态 Runtime；「⚙ 编译后运行」= 生成代码；执行触发由图与宿主推导；“真实时长”=主动推进的 pacing 子配置；本机/串口=访问端点。图内时钟域另见 `docs/HOW/reference/timeline.md`。
