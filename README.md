...
目前我就职于一家（可以算是）世界顶级的音频公司，在这里我学习到了音频处理的各种知识，也接触到了商业应用的各种音频模型。

公司有一套基于Simulink的可视化音频模型构建工具以及基于它之上的代码生成与调音系统。当然，由于商业限制，这一套工具显然是封闭的。

市面上也有类似的软件，比如AWE。它同样基于Matlab，而且，很不幸地，它的License非常贵，并且你想用它还得先有Matlab的License。

但我更忍受不了的是这类软件那简陋工业风的界面、臃肿的架构和代码、以及完全封闭的生态。我想要一个现代的、免费的、开放的、直观的工具，并且这个工具要有可玩性，而非只能在DSP设备上上部署。

我也调查了音频相关的开源项目和软件，但无一例外的，虽说专业性没问题，但在这个AI时代下还是显得太陈旧了。
...
我在设计Orpheus的时候充分考虑了教学以及商业应用上的特性，从一开始我就将其视为商业软件的挑战者。而且更cool的是，它并非是一个“工业风”的项目，它完全可以作为音频爱好者的玩具，融入日常的使用中。


### 现代的界面设计

我选择网页前端作为软件界面，让观感和手感更为柔和，避免“工业风”的感觉。

上方为功能菜单，下方为状态和log输出栏；左侧为组件，右侧为参数面板，左右均可折叠以及扩大显示区域。

![ui](docs/img_ui_main.png)


### 学习友好设计

所有组件都有说明文档，点击其右上角“!”标记即可打开。

![ui](docs/img_ui_components_detail.png)

同时工程和节点还支持项目级别的笔记，在项目笔记中可以描述该组件在本项目中的实际用途。

![ui](docs/img_ui_notes.png)

本项目内置各种观测组件，可以方便地看到音频在经过某个组件后产生的变化，直观地观察到组件的效果。

![ui](docs/img_ui_diagram.png)


### 可玩性设计

没错，这套工具可以直接在Windows上玩调音。你可以定制你自己的、在平时听歌中就能使用的音效模型！

当然，为本项目编写一个虚拟驱动是不明智的，在这里我们需要另外一个软件：[VB-CABLE](https://vb-audio.com/Cable/)

它实现了虚拟声卡的功能，我们只需要将播放器的输出指定为CABLE Input，同时在我们的“音频采集”（device_in）组件中指定采集设备为CABLE Input，Orpheus就能“接管”播放器的声音。

![ui](docs/img_win_audio_devices.png)

![ui](docs/img_ui_input_specify.png)

同时在“设备输出”（device_out）组件中指定输出的设备（如耳机），我们就能听到我们自己调制的音效了！
...
+ 统一的调试桥，可调试运行也可连接嵌入式设备

+ 框架及内置组件0动态分配内存

+ 针对调音参数进行设计的连续内存分布及大块内存数据传输机制


### AI友好设计

+ 文档齐全，以README.md + AGENTS.md，WHY、WHAT、HOW组织。

+ 提供SKILL，AGENT使用本系统0障碍。
...
cd ..; python scripts/benchmark_graph.py --nodes 128 --blocks 1000
```
...
**视频响度均衡**：导入 `examples/video_loudness_voice.yaml` 可把不同视频的播放电平拉近。`节目响度均衡器`采用实时 RMS 自动增益（不是标准 LUFS 扫描），后接 limiter 保护峰值。`voice_select.select=1` 为原声，改成 `2` 会通过 30ms 交叉淡化切入人声清晰分支；`n_way_mux` 可通用于任意两条同签名子链的平滑旁路。想学习内部原理可导入 `examples/video_loudness_voice_decomposed.yaml`，查看 `probe_rms → loudness_gain_control → gain` 三段及两条控制链。实际监听建议让视频输出到虚拟声卡，再由 Orpheus 输出到耳机，避免采集和播放使用同一设备造成回授或原声叠加。
