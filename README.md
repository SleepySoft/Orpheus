...
- [`docs/design_execution_model.md`](docs/design_execution_model.md) — 执行实现、时钟来源、推进节奏与访问端点的统一术语。
...
- **▶ 运行**（动态 Runtime）：按有效图自动选择时钟来源。含设备组件时由声卡回调驱动并持续到停止；无设备图走主机驱动批处理，默认全速完成。输入输出自由组合：系统声音→处理→WAV 就是录制，WAV→处理→声卡就是播放。
- **⚙ 编译后运行**（代码生成路径）：生成独立 C 工程 → 静态编译 → 运行；与动态加载路径的输出有逐字节一致性测试保障。

“真实时长”不是第三种执行方式，而是无设备批处理的**推进节奏**：关闭时全速处理，开启时按墙钟等待，便于连续观察 Probe。设备驱动天然按硬件节奏运行，不需要该选项。工具栏的本机/串口是**访问端点**，只决定 UI 连接哪个运行实例，不决定图如何执行。完整四轴模型见 `docs/design_execution_model.md`。
